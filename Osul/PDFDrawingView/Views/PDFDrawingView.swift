...
public class PDFDrawingView: UIView{
    ...
    public var lineWidth: CGFloat{
        get{
            return ConstantsPdfData.lineWidth + ConstantsPdfData.halfPointShift
        }
        set{
            ConstantsPdfData.lineWidth = max(5, newValue)
        }
    }
    /**
    ...
    */
    ...
    private var allPages = [PDFPageDisplayer]()
    ...
    */
    ...
    private func drawBeginning(holder: UIView, range: CountableRange<Int>){
      //  visiblePages.forEach({$0.removeFromSuperview()})
       // visiblePages.removeAll()
        var createPage = {(int: Int) in self.drawPage(number: int)}
        if (!self.allPages.isEmpty) {
            createPage = {int in self.allPages[int]}
        }
        for counter in range {
            self.visiblePages.append(createPage(counter))
        }
        if (self.allPages.isEmpty) {
            self.allPages.append(contentsOf: self.visiblePages)
            self.drawAllPages(already: range)
        }
    }
    
    private func drawAllPages(already: CountableRange<Int>) {
        for num in already.endIndex ..< document.pageCount {
            self.allPages.append(self.drawPage(number: num))
        }
    }
    ...
    @discardableResult
    private func drawPage(number: Int, scale: CGFloat? = nil) -> PDFPageDisplayer{
        guard let page = document.page(at: number) else {return PDFPageDisplayer(frame: CGRect.zero, page: nil)}
        let internalScale = scale ?? getScale(number: number)
...
//        }
    ...
    private func addToView<T: UIView>(_ view: T) -> T {
        self.backgroundHold.subviews[0].addSubview(view)
        return view
    }
    ...
    private func updateHorizontal(_ scrollView: UIScrollView) {
        if (visiblePages[0].pageNumber > 0){
            if (checkAbove(offset: scrollView.contentOffset.x / scrollView.zoomScale, test: offsets[visiblePages[0].pageNumber].topX)){
                ...
                self.visiblePages.insert(self.addToView(self.allPages[visiblePages[0].pageNumber - 1]), at: 0)
            }
        }
        if let last = visiblePages.last, (last.pageNumber < numberOfPages - 1){
            if (checkBelow(offset: scrollView.contentOffset.x / scrollView.zoomScale, test: offsets[last.pageNumber].bottomX)){
                ...
                self.visiblePages.removeFirst()
                self.visiblePages.append(self.addToView(self.allPages[last.pageNumber + 1]))
            }
        }
    ...
    private func updateVertical(_ scrollView: UIScrollView)  {
        if (visiblePages[0].pageNumber > 0){
            if (checkAbove(offset: scrollView.contentOffset.y / scrollView.zoomScale, test: offsets[visiblePages[0].pageNumber].topY)){
                self.visiblePages.last?.removeFromSuperview()
                self.visiblePages.removeLast()
                self.visiblePages.insert(self.allPages[visiblePages[0].pageNumber - 1], at: 0)
            }
        }
        if let last = visiblePages.last, (last.pageNumber < numberOfPages - 1){
            if (checkBelow(offset: scrollView.contentOffset.y / scrollView.zoomScale, test: offsets[last.pageNumber].bottomY)){
                self.visiblePages[0].removeFromSuperview()
                self.visiblePages.removeFirst()
                self.visiblePages.append(self.allPages[last.pageNumber + 1])
            }
        }
